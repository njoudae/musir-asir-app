...
const convexFunctions = {
  ...
  setTrackingStatus: makeFunctionReference('backend:setTrackingStatus'),
...
const PUBLIC_FILES = new Set([
  ...
  'musir-logo.jpg',
  'demo-identity.png', 'demo-vehicle-license.png', 'demo-company-permit.png',
...
function readStore() {
  try {
    ...
    const storedPoints = Array.isArray(data.crossingPoints) ? data.crossingPoints : [];
    data.crossingPoints = DEFAULT_POINTS.map((point) => ({
      ...(storedPoints.find((stored) => stored.id === point.id) || {}),
      ...point
    }));
    data.crossingEvents ||= [];
    return data;
...
async function sendOtp(phone, code) {
  const deliveryMode = process.env.OTP_DELIVERY_MODE || '';
  if (deliveryMode === 'onscreen') {
    return { provider: 'onscreen', accepted: true };
  }
...
function effectiveTicketExpiry(ticket) {
  const issuedExpiry = new Date(ticket.issuedAt).getTime() + TICKET_VALIDITY_MS;
  const storedExpiry = new Date(ticket.expiresAt).getTime();
...
function ticketStatus(ticket, now = Date.now()) {
  ...
  return new Date(effectiveTicketExpiry(ticket)).getTime() > now ? 'active' : 'expired';
}

function publicTicket(ticket) {
  return { ...ticket, expiresAt: effectiveTicketExpiry(ticket), status: ticketStatus(ticket) };
}
...
async function handleApi(req, res, url) {
  ...
  if (method === 'POST' && pathname === '/api/auth/request-otp') {
    ...
    try {
      ...
      const expose = process.env.OTP_DELIVERY_MODE === 'onscreen'
        || (process.env.NODE_ENV !== 'production' && process.env.DEV_OTP_EXPOSE !== 'false' && (process.env.SMS_PROVIDER || 'console') === 'console');
  ...
  if (method === 'POST' && pathname === '/api/tickets') {
    ...
    const ticketInput = {
      ...
      expiresAt: new Date(now.getTime() + TICKET_VALIDITY_MS).toISOString(),
      status: 'active',
  ...
  const trackingMatch = pathname.match(/^\/api\/tickets\/([^/]+)\/tracking$/);
  if (method === 'POST' && trackingMatch) {
    const body = await parseBody(req);
    if (typeof body.active !== 'boolean') return apiError(res, 400, 'حالة التتبع غير صحيحة');
    if (DATA_PROVIDER === 'convex') {
      try {
        await convexMutation(convexFunctions.setTrackingStatus, { externalUserId: user.id, ticketExternalId: trackingMatch[1], active: body.active });
    ...
    const ticket = store.tickets.find((item) => item.id === trackingMatch[1] && item.userId === user.id);
    if (!ticket) return apiError(res, 404, 'التذكرة غير موجودة');
    ticket.trackingActive = body.active;
    writeStore(store);
    return json(res, 200, { ok: true, active: body.active });
  }
  ...
  if (req.method === 'GET' && pathname === '/api/admin/dashboard') {
    ...
    return json(res, 200, {
      ...
      stats: {
        ...
        trackedTrucks: vehicles.filter((item) => item.status === 'active' && item.trackingActive && item.latestLocation).length,
...
function serveStatic(req, res, url) {
  ...
  const publicRoot = `${ROOT_DIR}${path.sep}`;
